...
**Manual (fill gaps, still ≤5–8):** empty/weak register fields; billing Proceed disabled; payment method required; search no results; sign out. Do not dump the OUT inventory into the CSV.

**Valid/invalid transitions covered without a ticket state machine:** empty cart → no checkout (UI-07); qty below/above bounds (UI-06); duplicate identity (UI-04); failed auth (UI-05); payment-check then invoice-create (UI-03); API invoice without token (API-07).
